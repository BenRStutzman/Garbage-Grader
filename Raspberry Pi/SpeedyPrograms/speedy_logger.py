    ...
    while True:
        ...
        if on_hours[0] < hours < on_hours[1]: # if during "daytime":
            ...
            if ard_output[0].isalpha(): # action from the arduino
                ...
                if action in ['food added', 'bin removed', 'scale reset', 'scales reset']:
                    ident += 1
                    name = str.format('%06d.jpg' % ident)

                    # tells the cluster to copy the current composite to the "incoming" folder to be graded
                    os.system("ssh BenS@10.3.60.101 cp GarbageGrader/RPiPics/current_composite.jpg"
                              "GarbageGrader/RPiPics/"  + name)
